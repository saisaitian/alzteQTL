...
echo "$sample quality control using fastp start at:" `date`
   fastp -i $raw_dir/$sample\_1.fq.gz -I $raw_dir/$sample\_2.fq.gz -o $sample.R1.fq.gz -O $sample.R2.fq.gz -z 4 -q 20 -u 40 -n 5 -j $sample.json -h $sample.html
echo "$sample quality control using fastp end at:" `date`
cd ../

echo "$sample start squire Map at:" `date`
   squire Map -1 01_Clean_data/$sample.R1.fq.gz -2 01_Clean_data/$sample.R2.fq.gz -r 101 -p 15 -n $sample -f squire_fetch
echo "$sample end squire Map at:" `date`

echo "$sample start squire Count at:" `date`
   squire Count -r 101 -p 15 -n $sample -c squire_clean -f squire_fetch 
